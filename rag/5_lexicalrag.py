...
def tokenize(text):
    # Fix for removing the special characters in indexing tokens. 
    # Example, Without this function it is taking "AuditEventType.PERMISSION_DENIED," as index and in query we will not give COMMA obviously, 
    # so BM25 exact match fails.
...
if __name__ == "__main__":
    ...
    for chunk in chunks:
        if "PERMISSION_DENIED" in chunk.page_content:
            print(chunk.page_content)

            print(
                [t for t in chunk.page_content.lower().split()
                    if "permission" in t]
            )
