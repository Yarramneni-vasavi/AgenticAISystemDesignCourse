...
def load_codebase(repo_path: str) -> list:
    ...
    for path in Path(repo_path).rglob("*.py"):
        ...
        # Inject filename as the first line of content
        content_with_source = f"# FILE: {path.name}\n{text}"
        docs.append(Document(page_content=content_with_source, metadata={"source": str(path)}))
    return docs
...
def build_agent(vector_store: Chroma):
   retriever_tool = create_retriever_tool(
       ...
       document_prompt=PromptTemplate.from_template(
        "SOURCE FILE: {source}\n\n{page_content}"), # FIXED THE FILENAME ISSUE
